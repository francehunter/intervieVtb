...
Делается step-тест. Из него эмпирически получается максимум интенсивности. Максимум * 0.9 и делается час. Если час стабилен, то это успешный макс.перф.
...
Способы сбора:
- бюрократический: заапрувить у менеджера
- бюрократический+: попросить у аналитика на стороне заказчика
- аналитический: взять из бд/логов статистику запросов. Выделить самые часто используемые. Дополнить теми, что существенно влияют на общий перформанс. 20% действий дают 80% результата.
...
Аппаратные метрики
- нагрузка и утилизация процессора (кол-во переключений контекста)
- использование памяти, прогрессия файла подкачки 
- нагрука на сетевые интерфейсы
- производительность работы дисковой подсистемы (длина очереди)
- метрики бд
...
Корреляция - оперирование данными, специфичными для данного времени. Например, id сессии сервера.
...
Запись.
- Перейти в Vugen, нажать кнопку "add script". Выбрать браузер и целевой метод, нажать кнопку "play". В открытом браузере совершать юзер активность. Нажать кнопку "stop". В результате будет нагенерен метод, который воспроизводит пользовательскую активность. 
- Написать метод руками.
- Импортировать из har файла.
...
collate - это процесс сбора локальных данных со всех рабочих станций, участвующих в тестировании, и их объединение для передачи анализатору.
...
Авто-корреляция - криво работающая автоматизация корреляции со стороны LR.
...
Cost - условная стоимость шага, которая расчитывается как (1*cpu) + (т1*hdd) + (m2*net). Полезно, если надо понять, почему вместо работы по индексу планировщик предпочел фулл сканить.

Options - full scan, index, cartesian.
...
Лаг - разница между позицией записи продьюсера и чтения консамера.
