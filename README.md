...
Kafka - это брокер сообщений, применяемый для обмена данными в микросервисной среде. Получает от "продьюсера" данные вида "ключ->значение" и кладет их а указанную очередь=топик. Множество "консамеров" могут эти значения из очереди извлекать.
